 ...
 *
 * Eaglermod integration: the title-screen update overlay also owns the Mods
 * entry button. Keeping the hook here avoids editing the fragile redacted
 * Minecraft GuiMainMenu patch.
 */
...
import net.lax1dude.eaglercraft.v1_8.eaglermod.gui.GuiEaglModList;
...
public class GuiUpdateCheckerOverlay extends Gui {
	...
	public void setResolution(Minecraft mc, int w, int h) {
		...
		this.width = w;
		this.height = h;
		modsButton = new GuiButton(90, w - 106, 4, 100, 20, "Mods");
		modsButton.visible = !isIngame;
		if(!UpdateService.supported()) return;
		checkForUpdatesButton = new GuiButton(0, 0, 0, 150, 20,
				I18n.format("update.button") + " " + I18n.format(mc.gameSettings.enableUpdateSvc ? "gui.yes" : "gui.no"));
	...
	public void drawScreen(int mx, int my, float partialTicks) {
		if(modsButton != null && !isIngame) modsButton.drawButton(mc, mx, my);
		if(!UpdateService.supported()) return;
		...
		if(mc.gameSettings.enableUpdateSvc) {
			...
			if(cert != null) {
				...
				int embedHeight = 44;
				if(lst != null) embedHeight += 3 + lst.size() * 6;
				...
				mc.fontRendererObj.drawString(I18n.format("update.update") + " " + EnumChatFormatting.DARK_RED
						+ cert.bundleDisplayVersion, 45, 25, 0x000000);
				if(lst != null) {
	...
	public void drawScreenBusy(int mx, int my, float partialTicks, UpdateProgressStruct progressState) {
		...
		if(!StringUtils.isAllBlank(progressState.statusString1))
			mc.fontRendererObj.drawStringWithShadow(progressState.statusString1, 3, 0, 0xFFFFFF);
		int cc = isIngame ? 0xBBBBBB : 0xFFFFFF;
		if(!StringUtils.isAllBlank(progressState.statusString2))
			mc.fontRendererObj.drawStringWithShadow(progressState.statusString2, 3, 11, cc);
		int progX1 = 3, progY1 = 22, progX2 = 135, progY2 = 32;
		float prog = progressState.progressBar;
		if(prog >= 0.0f) {
			int bk = 0xFFBBBBBB, fg = 0xFFDD0000;
		...
		if(!StringUtils.isAllBlank(progressState.statusString3)) {
			...
			List<String> wrappedURL = mc.fontRendererObj.listFormattedStringToWidth(progressState.statusString3,
					(int)((progX2 - progX1) * 1.5f));
			for(int i = 0, l = wrappedURL.size(); i < l; ++i)
				mc.fontRendererObj.drawStringWithShadow(wrappedURL.get(i), 5, i * 11, cc);
	...
	public void mouseClicked(int mx, int my, int btn) {
		if(btn == 0 && modsButton != null && !isIngame && modsButton.mousePressed(mc, mx, my)) {
			mc.displayGuiScreen(new GuiEaglModList(backScreen));
			return;
		}
		if(!UpdateService.supported()) return;
		if(btn == 0) {
			...
			if(startDownloadButton.mousePressed(mc, mx, my) && !UpdateService.getUpdatingStatus().isBusy) {
				UpdateCertificate cert = UpdateService.getLatestUpdateFound();
				if(cert != null) UpdateService.startClientUpdateFrom(cert);
			}
			if(viewAllUpdatesButton.mousePressed(mc, mx, my)) mc.displayGuiScreen(new GuiUpdateVersionList(backScreen));
			if(dismissUpdatesButton.mousePressed(mc, mx, my)) {
				UpdateCertificate cert = UpdateService.getLatestUpdateFound();
				if(cert != null) UpdateService.dismiss(cert);
			}
		}
	}
